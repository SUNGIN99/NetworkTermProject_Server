...
int main(int argc, char *argv[])
{
	...
	BOOL optval = TRUE;
	retval = setsockopt(listen_sock_UDPv4, SOL_SOCKET,
	...
	serveraddrUDPv4.sin_addr.s_addr = inet_addr("127.0.0.1");
	serveraddrUDPv4.sin_port = htons(SERVERPORT);
	retval = bind(listen_sock_UDPv4, (SOCKADDR*)&serveraddrUDPv4, sizeof(serveraddrUDPv4));
	if (retval == SOCKET_ERROR) err_quit("bind()");
	...
	SOCKET listen_sock_UDPv6 = socket(AF_INET, SOCK_DGRAM, 0);
	if (listen_sock_UDPv6 == INVALID_SOCKET) err_quit("socket()");

	// SO_REUSEADDR 소켓 옵션 설정
	BOOL optval = TRUE;
	retval = setsockopt(listen_sock_UDPv6, SOL_SOCKET,
		SO_REUSEADDR, (char*)&optval, sizeof(optval));
	if (retval == SOCKET_ERROR) err_quit("setsockopt()");

	SOCKADDR_IN6 serveraddrUDPv6;
	ZeroMemory(&serveraddrUDPv6, sizeof(serveraddrUDPv4));
	serveraddrUDPv6.sin6_family = AF_INET;
	serveraddrUDPv6.sin6_addr = in6addr_any;
	serveraddrUDPv6.sin6_port = htons(SERVERPORT);
	retval = bind(listen_sock_UDPv6, (SOCKADDR*)&serveraddrUDPv6, sizeof(serveraddrUDPv6));
	if (retval == SOCKET_ERROR) err_quit("bind()");
	/*----- UDP IPv6 소켓 초기화 끝 -----*/
	...
	while(1){
		...
		for(i=0; i<nTotalSockets; i++){
			...
			if(FD_ISSET(ptr->sock, &rset)){
				...
				printf("received bytes = %d", retval);
