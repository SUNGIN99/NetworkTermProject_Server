...
int main(int argc, char *argv[])
{
	...
	DWORD please = WaitForSingleObject(hThread[0], INFINITE);
...
DWORD WINAPI UDPv4_Multicast(LPVOID arg) {
	...
	mreq_v4.imr_multiaddr.s_addr = inet_addr(MCAST);
	...
	remoteaddr_v4.sin_addr.s_addr = inet_addr(MULTICASTIPv4);
...
DWORD WINAPI UDPv6_Multicast(LPVOID arg) {
	...
	while (1) {
		...
		retvalUDP = recvfrom(listen_sock_UDPv6, buf_UDP, BUFSIZE, 0,
			(SOCKADDR*)&peeraddr_v6, &addrlen_UDP); // peeraddr NULL 로 해도 ㄱㅊ
		if (retvalUDP == SOCKET_ERROR) {
			err_display("recvfrom()");
		}

		retvalUDP = sendto(listen_sock_UDPv6, buf_UDP, strlen(buf_UDP), 0,
